...
class ElementInterface {
  ...
  getActiveMarksAtRange(range) {
    ...
    let startText = this.getDescendantByPath(startPath)
    let endText = this.getDescendantByPath(endPath)
  ...
  getClosestByPath(path, iterator) {
    ...
    let p = PathUtils.lift(path)
    let found

    // Perf: no need to get the full list of ancestors if the match is earlier in the list
    while (!found && p.size) {
      const ancestor = this.getDescendantByPath(p)

      if (ancestor && iterator(ancestor)) {
        found = ancestor
      } else {
        p = PathUtils.lift(p)
      }
  ...
  /**
   * Get a descendant node by its path.
   *
   * @param {List} path
   * @return {Node|Null}
   */
  ...
  getFurthestByPath(path, iterator) {
    ...
    let node = this.nodes.get(path.first())
    let i = 0
    let furthest

    while (!furthest && node && i < path.size - 1) {
      if (node && iterator(node)) {
        furthest = node
      } else {
        i += 1
        node = node.nodes && node.nodes.get(path.get(i))
      }
    }
  ...
  getInsertMarksAtRange(range) {
    ...
    const text = this.getDescendantByPath(start.path)
  ...
  getMarksAtPosition(path, offset) {
    path = this.resolvePath(path)
    const text = this.getDescendantByPath(path)
  ...
  getOffset(path) {
    path = this.resolvePath(path)
    return this.getOffsetByPath(path)
  }

  getOffsetByPath(path) {
    ...
    const ret =
      path.size === 1
        ? offset
        : offset + this.nodes.get(index).getOffsetByPath(PathUtils.drop(path))
    return ret
  }
  ...
  getOffsetAtRange(range) {
    ...
    const { start } = range
    const offset = this.getOffsetByPath(start.path) + start.offset
    return offset
  }
  ...
  getOrderedMarksBetweenPositions(startPath, startOffset, endPath, endOffset) {
    ...
    const startText = this.getDescendantByPath(startPath)

    if (PathUtils.isEqual(startPath, endPath)) {
      return startText.getMarksBetweenOffsets(startOffset, endOffset)
    }

    const endText = this.getDescendantByPath(endPath)
  ...
  getSelectionIndexes(range, currentPath) {
    const { start, end } = range

    // if we've been given an invalid selection we can exit early.
    if (range.isUnset) return null

    // if we are in a nested node, not the document itself
    if (currentPath) {
      const cp = PathUtils.create(currentPath)
      const relativeToStart = PathUtils.compare(currentPath, start.path)
      const relativeToEnd = PathUtils.compare(currentPath, end.path)

      // If the range fully incorporates this path, return whole size
      if (relativeToStart === 1 && relativeToEnd === -1) {
        return { start: 0, end: this.nodes.size }
      }

      // If we are beyond the start / end, return null
      if (relativeToStart === -1 || relativeToEnd === 1) return null

      const startIndex = relativeToStart === 0 ? start.path.get(cp.size) : 0
      const endIndex =
        relativeToEnd === 0 ? end.path.get(cp.size) : this.nodes.size

      return { start: startIndex, end: endIndex }
    }

    return { start: start.path.get(0), end: end.path.get(0) + 1 }
  }
  ...
  getTextsBetweenPathPositionsAsArray(startPath, endPath) {
    ...
    // where we can avoid a lot of iterating of the tree.
    if (startPath && endPath && PathUtils.isEqual(startPath, endPath)) {
      return [this.getDescendantByPath(startPath)]
    } else if (!startPath && !endPath) {
